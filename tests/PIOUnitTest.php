...
final class PIOUnitTest extends \ExternalModules\ModuleBaseTest
{
	...
	public function testGetSiteAData() {
		...
		$this->assertIsObject($result, "REVERSE_LC->my_site_data is not an object after calling ->getMySiteData()");
		$this->assertEquals("001 - Site A", $result->site_name, "getMySiteData() didn't correctly determine site name property");
		$this->assertIsArray($result->rows, "REVERSE_LC->my_site_data->rows is not an object after calling ->getMySiteData()");
	...
	public function testGetAllSitesData() {
		...
		// ensure our result is structured as expected
		$this->assertIsObject($result, "REVERSE_LC->all_sites_data is not an object after calling ->getAllSitesData()");
		$this->assertIsArray($result->totals, "REVERSE_LC->my_site_data->totals is not an object after calling ->getAllSitesData()");
		$this->assertIsArray($result->sites, "REVERSE_LC->my_site_data->sites is not an object after calling ->getAllSitesData()");
